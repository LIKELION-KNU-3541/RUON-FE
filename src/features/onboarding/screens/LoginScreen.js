...
// 2p_top_circle.png: 348x660 ??116x220
// 2p_btm_circle.png: 270x999 ??90x333
...
export default function LoginScreen({ onNext }) {
  ...
  return (
    <View className="flex-1 bg-[#E1AF85] relative">
      <StatusBar style="light" />

      {/* 1. Base Background Image (login_bg.png) */}
      <ImageBackground
        source={require('../../../../assets/images/2p_bg.png')}
        className="flex-1 relative"
        resizeMode="cover"
      >
        {/* 2. 
        <LinearGradient colors={['rgba(147, 82, 27, 0)', '#945C2D']} locations={[0, 1.0]} style={{ position: 'absolute', left: 0, right: 0, top: 0, bottom: 0, zIndex: 1 }} />
        <View style={{ position: 'absolute', left: 0, right: 0, top: 0, bottom: 0, zIndex: 2 }}>
          <Image source={require('../../../../assets/images/2p_top_circle.png')} style={{ position: 'absolute', right: scaleW(-20), top: scaleH(-80), width: scaleW(116), height: scaleW(220) }} resizeMode="contain" />
          <Image source={require('../../../../assets/images/2p_btm_circle.png')} style={{ position: 'absolute', left: scaleW(-30), bottom: scaleH(-60), width: scaleW(90), height: scaleW(333) }} resizeMode="contain" />
        </View>
        */}

        {/* 4. Logo & Subtitle Header (top: 84px) */}
        <View style={{ position: 'absolute', left: 0, right: 0, top: scaleH(84), zIndex: 10 }} className="items-center">
          <SvgXml xml={ruonLogoSvgXml} width={scaleW(120)} height={scaleH(58)} />
          <Text style={{ marginTop: scaleH(16) }} className="text-[#FFF9F1] text-[14px] font-medium leading-[20px] text-center">
            {"임산부의 스킨케어\n루틴을 책임지는 루온입니다"}</Text>
        </View>
        ...
        <View style={{ position: 'absolute', left: scaleW(40), right: scaleW(40), top: scaleH(286), zIndex: 10 }}>
          ...
          {/* ID Input */}
          <View style={{ height: scaleH(50) }} className="w-full bg-white/10 rounded-[16px] border border-white/20 justify-center px-4 mb-4">
            <TextInput
              value={userId}
          ...
          >
            <Text className="text-[#945C2D] text-[16px] font-bold">로그인</Text>
          </TouchableOpacity>
        </View>
