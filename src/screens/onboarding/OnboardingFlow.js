...
export default function OnboardingFlow() {
  ...
  return (
    <View className="flex-1 bg-[#1E120A]">
      {/* Top Step Selector Bar for Easy Demonstration & Navigation */}
      <View className="bg-[#2B1D12] pt-10 pb-2 px-4 flex-row justify-between items-center border-b border-white/10 z-50">
        <TouchableOpacity
          onPress={prevStep}
          disabled={currentStep === 1}
          className={`px-3 py-1.5 rounded-lg border ${
            currentStep === 1 ? 'border-white/10 opacity-30' : 'border-white/30 bg-white/10'
          }`}
        >
          ...
          {[1, 2, 3, 4, 5, 6].map((num) => (
            <TouchableOpacity
              ...
              className={`w-6 h-6 rounded-full items-center justify-center mx-0.5 ${
                currentStep === num ? 'bg-white' : 'bg-white/15'
              }`}
            >
              <Text
                className={`text-xs font-bold ${
                  currentStep === num ? 'text-[#4A311F]' : 'text-white/70'
                }`}
              >
                {num}
              </Text>
        ...
        <TouchableOpacity
          ...
          className={`px-3 py-1.5 rounded-lg border ${
            currentStep === 6 ? 'border-white/10 opacity-30' : 'border-white/30 bg-white/10'
          }`}
        >
